...
export async function getTicketsData(userId: number) {
    try {
        ...
        if (!session || (session.userId !== userId && session.role !== 'ADMIN')) {
            return { success: false, message: "Não autorizado.", tickets: [] };
        }
...
export async function getBillingData(userId: number) {
    try {
        const session = await getSession();
        if (!session || (session.userId !== userId && session.role !== 'ADMIN')) {
            return { success: false, message: "Não autorizado.", orders: [], summary: { totalGasto: 0, totalPedidos: 0, totalBilhetes: 0 } };
        }
